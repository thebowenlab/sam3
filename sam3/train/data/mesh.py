# Mesh data infrastructure for CSE, adapted from detectron2/DensePose
...
def register_mesh(mesh_info: MeshInfo, base_path: Optional[str]) -> None:
    geodists = mesh_info.geodists
    if geodists:
        geodists = os.path.join(base_path, geodists)
    MeshCatalog[mesh_info.name] = MeshInfo(
        name=mesh_info.name,
        data=os.path.join(base_path, mesh_info.data),
        geodists=geodists,
    )
